...
Padrão de `clube-backend` (`keycloak-auth`, `administrators-admin`, `keycloak-partner-users-admin`). Frontend: skills `nextjs-keycloak-authorization` / `vite-keycloak-authorization`. Estrutura Nest: skill `nestjs-module-structure`.
