---
name: vite-keycloak-authorization
...
Padrão de `passin-frontend`. SPA **nunca** fala com Keycloak. BFF Nest monta URL, troca `code`, seta cookie. Sem `keycloak-js`, sem `oidc-client`. Axios + rotas: skill `api-auth`. M2M / Admin REST: skill `keycloak-admin-rest`.
...
Env Zod: `VITE_TOKEN_KEY` (nome do cookie, ex. `passin_token`), `VITE_COOKIE_DOMAIN`, `VITE_BACKEND_*`. App **não lê** o cookie. Consumir só via `env` / `backend_url` (skill `env-zod`).
